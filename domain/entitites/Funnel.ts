...
export class Funnel {
  ...
  moveNode(id: NodeId, position: NodePosition): DomainResult<void> {
    ...
    if (!node) {
      return DomainResultErrorFactory.createFunnelError(
        FunnelErrorCode.NODE_NOT_FOUND,
      );
    }
  ...
  connectNodes(from: NodeId, to: NodeId): DomainResult<void> {
    ...
    if (FunnelValidator.isExistingEdge(this.edges, from, to)) {
      return DomainResultErrorFactory.createFunnelError(
        FunnelErrorCode.EDGE_ALREADY_EXISTS,
      );
    }
  ...
  removeNode(id: NodeId): DomainResult<void> {
    const node = this.nodes.find((n) => n.id.equals(id));
    if (!node) {
      return DomainResultErrorFactory.createFunnelError(
        FunnelErrorCode.NODE_NOT_FOUND,
      );
    ...
    this.edges = this.edges.filter(
      (edge) => !edge.from.equals(id) && !edge.to.equals(id),
    );
    return { ok: true };
  }
